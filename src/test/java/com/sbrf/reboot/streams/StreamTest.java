...
public class StreamTest {
    ...
    public void filteredListStream() {
        ...
        List<Integer> actualIntegers = integers.stream().filter(it -> it % 2 == 0).sorted().collect(Collectors.toList());
    ...
    public void sortedAndFilteredBooks() {
        ...
        List<Book> actualBooks = books.stream()
                .filter(it -> it.author.equals("Maria"))
                .sorted(Comparator.comparing(it -> (it.price)))
    ...
    public void modifiedList() {
        ...
        List<String> actualContracts = contracts.stream().map(it -> "M-" + it).collect(Collectors.toList());
